...
from .generator import ABNet
from .discriminator import Discriminator
from .calculate_mos import calculate_mos
...
class Model(pl.LightningModule):
    def __init__(self, config):
        ...
        self.config = config
        self.generator = ABNet(**config['model_config'])
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=None):
        ...
        src, tgt, length, _ = batch
        channels = random.randint(1, src.size(1))
        src = src[:, :channels]
        results = self.generator(src, tgt)
        ...
        self.log_dict({'train_g_loss': g_loss, 'train_d_loss': d_loss}, on_step=True, on_epoch=True ,prog_bar=True)
        self.current_traning_step += 1
    ...
    def test_step(self, batch, batch_idx):
        ...
        sisdr_score = si_sdr(tgt[None,:], est[None,:])[0]
        self.log_dict({'test_pesq': pesq_score, 'test_stoi': stoi_score, 'test_sisdr': sisdr_score}, on_step=False, on_epoch=True)

        # pesq_score, stoi_score, csig, cbak, covl, ssnr = calculate_mos(tgt, est, 16000)
        # self.log_dict(
